...
Before installing anything, it's worth understanding what "installing a package" actually means. When you run `install.packages()`, R downloads the package and copies its files into a directory on your filesystem — this is your **package library**. You can see where R is currently looking for packages by running this in the **console** panel of Rstudio:
...
After installing a package you can literally go look at it by typing his in your hipergator terminal:
...
In R, you store data in objects using the assignment operator `<-`. Let's create a few to practice. Paste this in your R console and press enter:
...
Let's create a small practice file to read in. Run this in the **HiPerGator terminal**:
...
Now read it into R by pasting this in the R console (replace <username> with your username):

```r
sample_info <- read.table("/blue/bioinf_workshop/<username>/practice_samples.tsv", header=TRUE, sep="\t")
```
...
```r
library(tidyverse)
sample_info <- read_tsv("/blue/bioinf_workshop/<username>/practice_samples.tsv")
```
...
You can see the files you have created in your cwd or where you wrote them to if you used an explicit filepath:
